#include "dbscannote.h"

DbscanNote::DbscanNote ( Notes* d, type_dist t )
{
    this->donnees = d;
...
void DbscanNote::calculVoisinage ( unsigned int indice )
{
    this->voisinage_courant.clear();
    for (int i=0; i<(int)this->deja_visites.size();i++) {
        if (i == (int)indice)
            continue;
        if (this->distance.calculNote(this->lesDonnees->at(indice), this->lesDonnees->at(i)) < this->distance_maximum)
            this->voisinage_courant.push_back(i);
    }
}

void DbscanNote::etendCluster ( unsigned int indice )
{
